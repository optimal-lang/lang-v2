var omljs = (() => {
  ...
  function read_sexp(code, exp2) {
    ...
    switch (ch) {
      case "(":
      case "[":
        let lst = read_list(code, exp2, ch);
        return lst;
      case ")":
      case "]":
        return ch;
  ...
  function oml2ast(text2) {
    let code = tokenize(text2);
    let result = [];
    while (true) {
      let exp2 = [];
      let ast2 = read_sexp(code, exp2);
  ...
  var OMLCommon = class {
    ...
    is_id(ast2, name = void 0) {
      let ok = ast2 instanceof Array && ast2[0] === "#";
      if (!ok) return false;
      return name ? ast2[1] === name : true;
    }
    is_variable(ast2) {
      if (!(ast2 instanceof Array)) false;
    ...
    to_def(ast2) {
      ...
      switch (this.to_id(ast2[0])) {
        case "def": {
          if (ast2.length < 2) throw new Error("sysntax error");
          let ast1 = ast2[1];
          let ast22 = ast2.length === 2 ? null : ast2[2];
  ...
  function compile_ast(ast2) {
    ...
    switch (common.to_id(ast2[0])) {
      ...
      case "case": {
        ...
        let new_ast = [common.id("let*"), [[common.id("__case__"), ast2[1]]], cond_ast];
        return compile_ast(new_ast);
      }
      case "_cond": {
        let _cond_builder = function(rest) {
          if (rest.length === 0)
            return null;
        ...
        ast2 = _cond_builder(ast2.slice(1));
      ...
      case "dec!":
      case "inc!":
        let sign = common.to_id(ast2[0]) === "dec!" ? "-" : "+";
        let val2 = ast2.length < 3 ? 1 : compile_ast(ast2[2]);
      ...
      case "_let*": {
        ...
        for (let i = 1; i < ast2[1].length; i += 2) {
          ...
          let val3 = compile_ast(ast2[1][i]);
          if (i > 1)
            vals += ",";
          vals += val3;
          assigns += common.to_id(ast2[1][i - 1]) + "=" + val3 + ";";
        }
        vars += ")";
        vals += ")";
  ...
  function compile_do(ast2) {
    ...
    if (ast22.length < 2)
      ast22 = [ast22[0], null];
    let until_ast = [common.id("until"), ast22[0]].concat(ast2.slice(3));
    if (parallel) {
      ast1.forEach((x, i) => {
        if (x.length < 3)
          return;
  ...
  function omljs() {
    ...
    glob.compile_ast = (ast2, debug2) => {
      ...
      let code = compile_ast(ast2);
      if (debug2)
        console.log("[CODE] " + code);
      return code;
    };
    glob.compile = (text2, debug2) => {
      let steps2 = oml2ast(text2);
      let result = "";
    ...
    glob.exec = (exp, debug) => {
      ...
      for (let step of steps) {
        let exp = step[0];
        let ast = step[1];
        var tm1 = (/* @__PURE__ */ new Date()).getTime();
        try {
          if (debug)
          ...
          var tm2 = (/* @__PURE__ */ new Date()).getTime();
          if (debug) {
            if (output === void 0) {
